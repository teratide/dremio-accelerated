...
@Options
public class FletcherFilterProjectPrel extends ProjectRelBase implements Prel {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(ProjectPrel.class);

  public static final LongValidator RESERVE = new PositiveLongValidator("planner.op.fletcher_filter_project.reserve_bytes", Long.MAX_VALUE, DEFAULT_RESERVE);
  public static final LongValidator LIMIT = new PositiveLongValidator("planner.op.fletcher_filter_project.limit_bytes", Long.MAX_VALUE, DEFAULT_LIMIT);
  public static final boolean ALLOW_COMPLEX = true;
  public static final boolean ALLOW_GANDIVA_FUNCTIONS = true;

  protected FletcherFilterProjectPrel(RelOptCluster cluster, RelTraitSet traits, RelNode child, List<RexNode> exps, RelDataType rowType) {
    super(PHYSICAL, cluster, traits, child, exps, rowType);
  }

  @Override
  public FletcherFilterProjectPrel copy(RelTraitSet traitSet, RelNode input, List<RexNode> exps, RelDataType rowType) {
    return new FletcherFilterProjectPrel(getCluster(), traitSet, input, exps, rowType);
  }
  ...
  public <T, X, E extends Throwable> T accept(PrelVisitor<T, X, E> logicalVisitor, X value) throws E {
    return logicalVisitor.visitFletcherFilterProject(this, value);
  }
   ...
   */
  public static FletcherFilterProjectPrel create(RelOptCluster cluster, RelTraitSet traits, RelNode child, List<RexNode> exps,
                                   RelDataType rowType) {
    final RelTraitSet adjustedTraits = adjustTraits(cluster, child, exps, traits);
    return new FletcherFilterProjectPrel(cluster, adjustedTraits, child, exps, rowType);
  }
