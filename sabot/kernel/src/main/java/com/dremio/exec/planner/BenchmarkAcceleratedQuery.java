...
public class BenchmarkAcceleratedQuery extends BaseTestQuery {
  ...
  public static void main(String[] args) throws Exception {
    ...
    createCSV("filter_tidre.csv");
    createCSV("parquet_tidre.csv");
  ...
  public static void testInBenchmark() throws Exception {
    ...
    int repeats = 10;
    String[] in_sizes = {"1000", "2000", "4000", "8000", "16000", "32000", "64000", "128000", "256000", "512000"};
    ...
    for (int i = 0; i < in_sizes.length; i++) {
      ...
      for (int r = 0; r < repeats; r++) {
        ...
        writeRecordsRepeatsToCSV("filter_tidre.csv", in_sizes[i], String.valueOf(r));
        writeRecordsRepeatsToCSV("parquet_tidre.csv", in_sizes[i], String.valueOf(r));
        test(query);
      }
    }
  }
  ...
  public static void testBatchBenchmark() throws Exception {
    ...
    int repeats = 10;
    String query = "SELECT SUM(\"value\") FROM cp.\"diving/data-32000-1M.parquet\" WHERE REGEXP_LIKE(\"string\", '.*[tT][eE][rR][aA][tT][iI][dD][eE][ \\t\\n]+[dD][iI][vV][iI][nN][gG][ \\t\\n]+([sS][uU][bB])+[sS][uU][rR][fF][aA][cC][eE].*')";
    String[] batch_sizes = {"1000", "2000", "4000", "8000", "16000", "32000", "64000"};

    writeBenchmarkName("filter_tidre.csv", "BATCH BENCHMARK");
    writeBenchmarkName("parquet_tidre.csv", "BATCH BENCHMARK");
    ...
    for (int i = 0; i < batch_sizes.length; i++) {
      ...
      for (int r = 0; r < repeats; r++) {
        System.out.println(String.valueOf(r));
        writeRecordsRepeatsToCSV("filter_tidre.csv", batch_sizes[i], String.valueOf(r));
        writeRecordsRepeatsToCSV("parquet_tidre.csv", batch_sizes[i], String.valueOf(r));
        test(query);
      }
    }
  }
